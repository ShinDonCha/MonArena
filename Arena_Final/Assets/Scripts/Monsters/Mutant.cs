...
public class Mutant : CmnMonCtrl
{
    ...
    protected override void UltiSkill()     //궁극기 : 전방으로 빠르게 이동 후 경로에있는 적 공격
    {
        ...
        navAgent.isStopped = true;      //네비에 의한 이동 막기
    ...
    {
        List<CmnMonCtrl> a_CMCList = myTeam switch
        {
            Team.Ally => FindClass.GetCMCListFunc(Team.Enemy.ToString()),
            Team.Enemy => FindClass.GetCMCListFunc(Team.Ally.ToString()),
            Team.Team1 => FindClass.GetCMCListFunc(Team.Team2.ToString()),
            Team.Team2 => FindClass.GetCMCListFunc(Team.Team1.ToString()),
            _ => null
        };        

        if (a_CMCList.Count == 0 || a_CMCList == null)   //탐색된 적 몬스터 리스트가 없을경우 취소
            return;
