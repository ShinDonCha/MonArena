...
{   
    ...
    {
        //--------------------- 싱글에서만 실행 ---------------------
        if (ResultData.PrevScene != SceneList.MultiGameScene && Enum.TryParse(tag, out Team TeamTag))
        {
            switch (TeamTag)
            {
                case Team.Ally:
                ...
                case Team.Enemy when ResultData.PrevScene.Equals(SceneList.InGameScene):
                    //랭크 정보에 맞는 스탯 가져오기
                ...
                case Team.Enemy when ResultData.PrevScene.Equals(SceneList.RankGameScene):
                    //랭크 정보에 맞는 스탯 가져오기
    ...
    public void SetMonStat(int MonNameIndex, int StarForce, Team TeamTag)
    {
        ...
        hpImg.color = TeamTag switch   //팀에 따라 HP색상 변경
        {
