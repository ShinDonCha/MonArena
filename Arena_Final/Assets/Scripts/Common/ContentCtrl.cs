...
public class ContentCtrl : MonoBehaviour, IPointerDownHandler
{
    ...
    {
        if (!System.Enum.TryParse(SceneManager.GetActiveScene().name, out SceneList curScene))  //현재 씬의 이름을 SceneList 형식으로 변환(실패 시 함수 종료)
            yield break;

        yield return new WaitForFixedUpdate();      //Update() 전에 실행되도록

        List<MonsterName> a_Deck = new();           //저장된 덱 리스트를 받을 변수
        List<int> a_StarF = new();                  //저장된 덱의 성급 리스트를 받을 변수
