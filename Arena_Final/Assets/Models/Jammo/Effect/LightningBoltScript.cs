...
public class LightningBoltScript : MonoBehaviour
{
    ...
    public void SetPos(GameObject Mon, GameObject Enemy, GameObject HitEffect)
    {
        GameObject a_hitPoint = Enemy.GetComponent<CmnMonCtrl>().GetHitPoint.gameObject;
        StartObject = Mon;
        EndObject = a_hitPoint;
        LastStartPos = Mon.transform.position;
        LastEndPos = a_hitPoint.transform.position;

        if(HitEffect != null)
            Instantiate(HitEffect, a_hitPoint.transform.position, Quaternion.identity, FindClass.AreaTrFunc(transform.parent.tag));
    }
